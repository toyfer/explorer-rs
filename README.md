...
## Windows 用 .exe の入手 — 手動タグは不要

`main` へ push / merge すると **自動で Release が作成**されます。手で `git tag` する必要はありません。

- **自動:** `main` push → Cargo.toml の `version` から `vX.Y.Z` を解決 → 同じタグが無ければ安定 Release、既に存在すれば `vX.Y.Z-nightly-<sha>` の prerelease
- **手動（任意）:** `git tag v0.1.5 && git push origin v0.1.5` でも従来通り Release
...
`main` への push はそのまま自動 Release まで行きます。
