...
impl ExplorerApp {
    ...
    fn poll_bg(&mut self, ctx: &egui::Context) {
        ...
        for ev in events {
            match ev {
                ...
                BgEvent::PasteDone { message, ok, err } => {
                    self.pasting = false;
                    if err > 0 {
                        self.last_error = Some(format!("失敗 {err} 件 (成功 {ok})"));
                    }
                    self.status = message;
                    self.current_tab_mut().refresh();
                    if let Some(p) = self.pane2.as_mut() {
                        p.refresh();
                    }
                    self.update_preview();
    ...
    fn do_rename(&mut self) {
        ...
        let Some(new_name) = fs_ops::sanitize_filename(&self.rename_buffer) else {
            self.status = "不正な名前です（\\/ や .. は使えません）".into();
            return;
        };
    ...
    fn ui_display_settings(&mut self, ui: &mut egui::Ui, ctx: &egui::Context) {
        ui.collapsing("表示", |ui| {
            ...
            // egui 0.28 API: from_id_source (not from_id_salt)
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        if let Some(conf) = self.confirm_delete.clone() {
            ...
            .show(ctx, |ui| {
                ...
                ui.separator();
                ui.horizontal(|ui| {
                    let yes = ui.button(if permanent {
                        "完全に削除"
                    } else {
                        "ごみ箱へ"
                    });
        ...
        egui::TopBottomPanel::top("top_bar").show(ctx, |ui| {
            ui.horizontal_wrapped(|ui| {
                ...
                ui.add_enabled_ui(fwd_ok, |ui| {
                    if ui.button("▶ 進む").clicked() {
                        self.run_command(Command::GoForward);
                    }
                });
                if ui.button("⬆ 上へ").on_hover_text("Alt+↑").clicked() {
                    self.run_command(Command::GoUp);
                }
                if ui.button("⟳ 更新").on_hover_text("F5").clicked() {
                    self.run_command(Command::Refresh);
                }
                ui.separator();
                if ui.button("＋ タブ").on_hover_text("Ctrl+T").clicked() {
                    let cur = self.current_tab().current.clone();
                    let sh = self.show_hidden;
                    let sb = self.current_tab().sort_by;
                    let sd = self.current_tab().sort_desc;
                ...
                if (sresp.lost_focus() && ui.input(|i| i.key_pressed(egui::Key::Enter)))
                    || ui.small_button("検索").clicked()
                {
                    self.run_command(Command::Search);
                }
                if !self.search_query.is_empty() && ui.small_button("クリア").clicked() {
                    self.search_query.clear();
                    self.search_results.clear();
                }
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    let mut dual = self.dual_pane;
                    if ui.checkbox(&mut dual, "2ペイン").changed() {
                        self.set_dual_pane(dual);
                    }
                    let mut hidden = self.show_hidden;
                    if ui.checkbox(&mut hidden, "隠しファイル").changed() {
        ...
        egui::TopBottomPanel::bottom("status_bar").show(ctx, |ui| {
            ui.horizontal(|ui| {
                ...
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    let sel = self.current_tab().selected.len();
                    if sel > 0 {
                        ui.label(format!("選択: {sel}"));
                    }
                    ui.label(format!("{} 項目", self.current_tab().entries.len()));
                    if let Some(err) = &self.current_tab().error {
                        ui.colored_label(egui::Color32::from_rgb(220, 80, 80), err);
                    }
                    if let Some(cb) = &self.clipboard {
                        ui.label(format!("CB:{} {:?}", cb.paths.len(), cb.mode));
                    }
                });
            });
        });
                            ...
                            if ["png", "jpg", "jpeg", "gif", "bmp", "webp"].contains(&ext.as_str())
                            {
                                ...
                                ui.add(
                                    egui::Image::from_uri(uri)
                                        .max_size(egui::vec2(280.0, 280.0)),
                                );
                            }
                        }
...
fn icon_for(entry: &FileEntry) -> &'static str {
    ...
    match entry.ext.as_str() {
        ...
        "zip" | "7z" | "rar" | "tar" | "gz" => "📦",
        "exe" | "msi" | "dll" => "⚙️",
        "rs" | "toml" | "json" | "py" | "js" | "ts" | "go" | "c" | "cpp" | "h" | "cs" => "📝",
        _ => "📄",
    }
}
